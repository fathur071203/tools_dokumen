...
class SpreadsheetTrackingService:
    ...
    PENDING_CACHE_TTL_SECONDS = 25
    _pending_users_cache: list[dict[str, Any]] = []
    _pending_users_cache_at: float = 0.0

    @classmethod
    def register_user(cls, identity: LoginIdentity, password: str) -> tuple[bool, str]:
        spreadsheet = cls._open_spreadsheet()
        users_sheet = cls._get_or_create_worksheet(
            spreadsheet,
            cls.USERS_SHEET,
            cls.USER_HEADERS,
        )

        existing_row_number, existing_data = cls._find_user_row_by_username(
            worksheet=users_sheet,
            username=identity.username,
        )
        if existing_row_number:
            status = str(existing_data.get("status", "active") or "active")
            return (
                False,
                f"Username {identity.username} sudah terdaftar dengan status {status}.",
            )

        existing_row_number, existing_data = cls._find_user_row_by_email(
            worksheet=users_sheet,
        ...
        if existing_row_number:
            ...
            return (
                False,
                f"Email {identity.email} sudah terdaftar dengan status {status}.",
            )
        ...
        if is_admin_account:
            cls._invalidate_pending_users_cache()
    ...
    def verify_user_credentials(
        ...
        username: str,
        password: str,
    ) -> tuple[bool, str, LoginIdentity | None]:
        spreadsheet = cls._open_spreadsheet()
        users_sheet = cls._get_or_create_worksheet(
            spreadsheet,
            cls.USERS_SHEET,
        ...
        _, existing_data = cls._find_user_row_by_username(
            worksheet=users_sheet,
            username=username,
        )
        ...
        raw_status = str(existing_data.get("status", "") or "").strip().lower()
        status = raw_status or cls.USER_STATUS_ACTIVE
    ...
    @classmethod
    def list_pending_users(cls, force_refresh: bool = False) -> list[dict[str, Any]]:
        if not force_refresh and cls._is_pending_users_cache_valid():
            return list(cls._pending_users_cache)
            ...
            pending.append(
                {
                    ...
                    "role": str(row.get("role", cls.USER_ROLE_USER) or cls.USER_ROLE_USER).strip().lower(),
                }
            )
        cls._pending_users_cache = list(pending)
        cls._pending_users_cache_at = time.time()
        return pending
    ...
    def decide_user_approval(
        ...
        row_number, existing_data = cls._find_user_row_by_username(
            worksheet=users_sheet,
            username=username,
        )
        if not row_number or not existing_data:
            return False, f"User {username} tidak ditemukan."

        current_status = str(existing_data.get("status", "") or "").strip().lower()
        if current_status != cls.USER_STATUS_PENDING:
            return False, f"User {username} tidak dalam status pending (status saat ini: {current_status or '-'})."

        now = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")
        new_status = cls.USER_STATUS_ACTIVE if approve else cls.USER_STATUS_REJECTED
        role = str(existing_data.get("role", "") or "").strip().lower() or cls.USER_ROLE_USER

        users_sheet.update(
            f"A{row_number}:K{row_number}",
            [
                [
                    str(existing_data.get("registered_at", "") or "").strip() or now,
                    str(existing_data.get("last_login_at", "") or "").strip(),
                    str(existing_data.get("full_name", "") or "").strip(),
                    str(existing_data.get("username", "") or "").strip().lower(),
                    str(existing_data.get("email", "") or "").strip().lower(),
                    str(existing_data.get("unit", "") or "").strip(),
                    new_status,
                    str(existing_data.get("password_hash", "") or "").strip(),
                    role,
        ...
        cls._invalidate_pending_users_cache()
        return True, f"User {username} berhasil ditolak."

    @classmethod
    def _is_pending_users_cache_valid(cls) -> bool:
        if not cls._pending_users_cache:
            return False
        if cls._pending_users_cache_at <= 0:
            return False
        return (time.time() - cls._pending_users_cache_at) <= cls.PENDING_CACHE_TTL_SECONDS

    @classmethod
    def _invalidate_pending_users_cache(cls) -> None:
        cls._pending_users_cache = []
        cls._pending_users_cache_at = 0.0
    ...
    def register_and_log_login(cls, identity: LoginIdentity) -> tuple[bool, str]:
        ...
        access_log_sheet = cls._get_or_create_worksheet(
            spreadsheet,
            cls.ACCESS_LOG_SHEET,
            cls.ACCESS_LOG_HEADERS,
        )
    ...
    @classmethod
    def _find_user_row_by_username(
        cls,
        worksheet: gspread.Worksheet,
        username: str,
    ) -> tuple[int | None, dict]:
    ...
    def _open_spreadsheet(cls) -> gspread.Spreadsheet:
        ...
        if not cls.CREDENTIALS_PATH.exists():
            raise FileNotFoundError(
                "File credentials Google tidak ditemukan di static/credentials.json"
            )
        ...
        spreadsheet_name = os.getenv(
            cls.ENV_SPREADSHEET_NAME,
            cls.DEFAULT_SPREADSHEET_NAME,
        ).strip()
        try:
